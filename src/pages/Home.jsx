      ...
      <section className="bg-[#F9FAFB] py-20">
        <div className="container-custom">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight text-[#111827]">
                Turn NPAs Into Cash—<span className="text-[#D4AF37]">Fast & Legally</span>
              </h1>
              <p className="text-xl mb-8 text-[#374151]">
              ...
              <div className="flex flex-wrap gap-6 mb-8">
                ...
                <div className="flex items-center gap-2">
                  <CheckCircle className="text-[#D4AF37]" />
                  <span className="text-[#111827] font-semibold">SARFAESI | NCLT | DRT Experts</span>
                </div>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
            {problems.map((problem, idx) => (
              <AnimatedCard 
                ...
                direction="up"
              >
                <problem.icon className="w-12 h-12 text-[#D4AF37] mx-auto mb-4" />
                <h3 className="text-xl font-bold mb-2 text-[#111827]">{problem.title}</h3>
                <p className="text-[#374151]">{problem.desc}</p>
              </AnimatedCard>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
            {solutions.map((solution, idx) => (
              <AnimatedCard key={idx} className="h-full" delay={idx * 80} direction="up">
                ...
                >
                  <solution.icon className="w-12 h-12 text-[#D4AF37] mb-4" />
                  <h3 className="text-xl font-bold mb-2 text-[#111827] group-hover:text-[#D4AF37] transition-colors">
                    {solution.title}
                  </h3>
                  <p className="text-[#374151] mb-4">{solution.desc}</p>
                  <div className="text-[#D4AF37] font-semibold flex items-center gap-1 group-hover:gap-2 transition-all">
            ...
            <div className="flex justify-between items-start relative max-w-5xl mx-auto">
              {["Case Evaluation", "Strategy Planning", "Execution", "Recovery & Closure"].map((step, idx) => (
                <AnimatedCard 
                  ...
                  direction="up"
                >
                  <div className="w-16 h-16 bg-[#D4AF37] rounded-full flex items-center justify-center text-[#111827] font-bold text-2xl mx-auto mb-3 shadow-lg">
                    {idx + 1}
                  </div>
                  <span className="font-semibold text-lg block text-[#111827]">{step}</span>
                </AnimatedCard>
              ))}
              ...
              <div key={idx} className="text-center">
                ...
                {idx < 3 && (
                  <div className="flex justify-center my-4">
                    <svg className="w-6 h-6 text-[#D4AF37] transform rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
            {trackRecordStats.map((result, idx) => (
              <AnimatedCard key={idx} className="bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all border border-[#D4AF37]/20 hover:border-[#D4AF37]" delay={idx * 100} direction="up">
                <div className="text-3xl md:text-4xl font-bold text-[#D4AF37] mb-2">
                  {result.amount}
                </div>
                <h3 className="text-lg font-bold text-[#111827] mb-1">{result.client}</h3>
                <p className="text-sm text-[#D4AF37] font-semibold mb-2">{result.type}</p>
                <p className="text-[#374151] text-sm">{result.description}</p>
              </AnimatedCard>
            ))}
          </div>
        ...
        <div className="container-custom">
          ...
          <AnimatedSection direction="up" threshold={0.3}>
            <div className="max-w-4xl mx-auto text-center mb-12">
              <p className="text-[#374151] mb-4 leading-relaxed">
                We are empanelled with leading <strong className="text-[#D4AF37]">Nationalised Banks</strong>, 
                <strong className="text-[#D4AF37]"> Private Sector Banks</strong>, and 
                <strong className="text-[#D4AF37]"> Asset Reconstruction Companies (ARCs)</strong> across key financial hubs such as 
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {institutionalPartners.map((item, idx) => (
              <AnimatedCard key={idx} className="group bg-[#F9FAFB] rounded-xl p-6 text-center shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1" delay={idx * 100} direction="up">
                <item.icon className="w-12 h-12 text-[#D4AF37] mx-auto mb-3 group-hover:scale-110 transition-transform" />
        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
            {whyChooseItems.map((item, idx) => (
              <AnimatedCard key={idx} className="bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all hover:-translate-y-1 border border-[#D4AF37]/20 hover:border-[#D4AF37]" delay={idx * 100} direction="up">
          ...
          <AnimatedCard delay={500} direction="up" className="mt-12 text-center bg-[#111827] text-white rounded-2xl p-10 shadow-2xl">
            <svg className="w-12 h-12 text-[#D4AF37] mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p className="text-[#D4AF37] text-lg">Our Philosophy - Results-Driven Excellence</p>
          </AnimatedCard>
        </div>
      </section>
      ...
      <section className="py-16 bg-white">
        <div className="container-custom text-center">
          <AnimatedSection direction="up" threshold={0.3}>
            ...
            <p className="text-xl mb-8 text-[#374151] max-w-2xl mx-auto">
